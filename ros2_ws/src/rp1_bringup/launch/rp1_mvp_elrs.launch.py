...
elrs_node -> /joy -> rp1_teleop -> rp1_control -> rp1_dronecan_bridge.

Identical to rp1_mvp.launch.py except the /joy source: there is no joy_node here; rp1_elrs's
elrs_node reads the CRSF link and publishes /joy itself (and pushes battery telemetry back to the
handset). Each node loads its package's own default config first, then rp1_bringup's rp1_mvp.yaml
on top for the robot-specific values (serial port, track width, CAN interface, deadman button).
